...
from commands in the cpptraj input file."""
import re
import os
import sys
...
class CpptrajInputData(SinglefileData):
    """Class to find the inputs used and outputs produced from
    the commands in the cpptraj input file"""
    ...
    def set_file(self, file, filename=None, **kwargs):
        ...
        # Parse the cpptraj file
        parsed_info = parse_cpptraj_input_file(self.get_content().splitlines())
    ...
    def inpfile_list(self):
        """Return the list input files used in the cpptraj script
        """
        return self.base.attributes.get('input_files')
    
    @property
    def outfile_list(self):
        """Return the list output files to be produced from the cpptraj script
        """
        return self.base.attributes.get('output_files')

    @property
    def calculation_inputs_outputs(self):
        """Return the inputs for the cpptraj calculation job
        """
...
def parse_cpptraj_input_file(lines):
    """Parse cpptraj script and find any instances of file inputs or outputs
    ...
    input_files = []
    output_files = []
    # iterate through cpptraj lines and find input and output files
    for line in lines:
...
def add_calculation_inputs(subdirs, files):
    """If they exist, add input files for cpptraj and dirs into the calcjob 
    inputs directory
    """
    calc_inputs = {}
    input_list = []
    # If we have cpptraj input files then tag them.
    if files:
        calc_inputs["cpptraj_inpfiles"] = {}
        ...
        for file in files:
            ...
            if os.path.isfile(file):
                ...
                calc_inputs["cpptraj_inpfiles"][formatted_filename] = \
                    SinglefileData(file=os.path.join(os.getcwd(), file))

            elif "PYTEST_CURRENT_TEST" in os.environ:
                test_path = os.path.join(os.getcwd(), 
                                        'tests/input_files/cpptraj', file)
                if os.path.isfile(test_path):
                    calc_inputs["cpptraj_inpfiles"][formatted_filename] = \
                        SinglefileData(file=test_path)
                else:
                    sys.exit(f"Error: Input file {file} referenced in cpptraj file does not exist")
    ...
    if subdirs:
        ...
        for subdir in subdirs:
            if os.path.isfile(subdir):
                ...
                if frst_dir not in calc_inputs["cpptraj_dirs"].keys():
                    calc_inputs["cpptraj_dirs"][frst_dir] = FolderData()
                # Now fill it with files referenced in the cpptraj inputfile.
                # need to make sure to include any nested dirs in the path
                calc_inputs["cpptraj_dirs"][frst_dir].put_object_from_file(
            ...
            # For tests
                ...
                if os.path.isfile(os.path.join(os.getcwd(), "tests", subdir)):
                    # Create a folder that is empty.
                    if "tests" not in calc_inputs["cpptraj_dirs"].keys():
                        calc_inputs["cpptraj_dirs"]["tests"] = FolderData()
                    # Now fill it with files referenced in the cpptraj inputfile.
                    calc_inputs["cpptraj_dirs"]["tests"].put_object_from_file(
                        os.path.join(os.getcwd(), "tests", subdir), 
                        path=subdir)
                        
            else:
                sys.exit(f"Error: subdir {subdir} referenced in cpptraj file does not exist")
...
def add_calculation_outputs(files):
    """Add outputs from cpptraj script
    """
    calc_outputs = {}
    # If we have cpptraj output files then tag them.
    if files:
        output_list = []
        # Iterate files to assemble a dict of names and paths.
        for file in files:
        ...
        calc_outputs["cpptraj_outfiles"] = List(output_list)
    return calc_outputs
