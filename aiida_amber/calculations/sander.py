...
class SanderCalculation(CalcJob):
    ...
    def define(cls, spec):
        ...
        spec.input('metadata.options.output_filename', valid_type=str, default='sander.out')
        spec.input('parameters', valid_type=SanderParameters, help='Command line parameters for sander')
        ...
        spec.output("mdout", valid_type=SinglefileData, 
                    help="output user readable state info and diagnostics -o stdout will send output to stdout (to the terminal) instead of to a file.")
        ...
        spec.output("suffix", valid_type=str, required=False, 
                    help="output this string will be added to all unspecified output files that are printed (for multisander runs, it will append this suffix to all output files).")
    ...
    def prepare_for_submission(self, folder):
        ...
        input_options = ["mdin", "prmtop", "inpcrd", "refc", "mtmd", "inptraj", 
                    "inpdip", "cpin", "cein", "evbin"]
        output_options = ["o", "inf", "x", "v", "frc", "e", "r", "rdip", 
                    "cprestrt", "cpout", "cerestrt", "ceout", "suffix"]
        cmdline_input_files = {}
        input_files = []
        output_files = []
